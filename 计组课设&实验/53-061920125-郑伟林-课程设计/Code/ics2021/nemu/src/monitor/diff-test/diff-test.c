...
void difftest_step(uint32_t eip) {
  ...
  {
      diff = true;
      printf("NEMU don't match QEMU at $eip=0x%08x !\nQEMU              NEMU\n",cpu.eip);
      printf("$eax=0x%08x   $eax=0x%08x\n",r.eax,cpu.eax);
      printf("$ebx=0x%08x   $ebx=0x%08x\n",r.ebx,cpu.ebx);
      printf("$ecx=0x%08x   $ecx=0x%08x\n",r.ecx,cpu.ecx);
      printf("$edx=0x%08x   $edx=0x%08x\n",r.edx,cpu.edx);
      printf("$esp=0x%08x   $esp=0x%08x\n",r.esp,cpu.esp);
      printf("$ebp=0x%08x   $ebp=0x%08x\n",r.ebp,cpu.ebp);
      printf("$esi=0x%08x   $esi=0x%08x\n",r.esi,cpu.esi);
      printf("$edi=0x%08x   $edi=0x%08x\n",r.edi,cpu.edi);
      printf("$eip=0x%08x   $eip=0x%08x\n",r.eip,cpu.eip);
      printf("$eflag=0x%08x $eflag=0x%08x\n",r.eflags,cpu.eflags.val);
  }
